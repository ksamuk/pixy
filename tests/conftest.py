...
@pytest.fixture()
def ag1000_gvcf_path(datadir: Path) -> Path:
    """
    Path to a GVCF derived from the ag1000 all-sites fixture.

    Generated by ``tests/main/data/scripts/build_gvcf_fixture.py``: maximal runs of
    consecutive invariant (``ALT='.'``) records in the all-sites fixture are
    collapsed into single block records with ``ALT=<NON_REF>`` and ``INFO/END``.
    The regression test ``test_pixy_gvcf_matches_all_sites_baseline`` asserts that
    pi/dxy/fst output on this GVCF matches the output on the all-sites fixture.
    """
    return datadir / "ag1000_pixy_gvcf_test.vcf.gz"
...
def run_pixy_helper(  # noqa: C901
    ...
    gvcf: bool = False,
    gvcf_max_block_size: Optional[int] = None,
) -> None:
    """
    ...
    if gvcf:
        test_args.extend(["--gvcf"])

    if gvcf_max_block_size is not None:
        test_args.extend(["--gvcf_max_block_size", f"{gvcf_max_block_size}"])
